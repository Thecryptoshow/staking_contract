...
contract ShowStaking is Owned
{
    ...
    uint256 public totalStakedShow;
    ...
    function pendingReward(uint256 _poolId, address _user) external view returns (uint256) 
    {
        ...
        uint256 supply;
        if (_poolId == 0)
            supply = totalStakedShow;
        else
            supply = pool.token.balanceOf(address(this));
        uint256 balance = rewardToken.balanceOf(address(this)).sub(totalStakedShow);
    ...
    {
        PoolInfo storage pool = poolInfo[_poolId];
        uint256 rewardBalance = rewardToken.balanceOf(address(this)).sub(totalStakedShow);
    ...
    function deposit(uint256 _poolId, uint256 _amount) public 
    {
        ...
        if (_amount > 0) {
            ...
            if (_poolId == 0)
            totalStakedShow = totalStakedShow.add(_amount);
        }
    ...
    function withdraw(uint256 _poolId, uint256 _amount) public 
    {
        ...
        if (_amount > 0) {
            ...
            if (_poolId == 0)
            totalStakedShow = totalStakedShow.sub(_amount);
        }
        user.rewardDebt = user.amountStaked.mul(pool.accRewardPerShare).div(1e12);
    ...
    function emergencyWithdraw(uint256 _poolId) public 
    {
        ...
        if (_poolId == 0)
        totalStakedShow = totalStakedShow.sub(amount);
        emit EmergencyWithdraw(msg.sender, _poolId, amount);
    }
    
    // emergency show transfer function, just in case if rounding error causes pool to not have enough SHOWs.
    function emergencyShowTransfer(address _to) external ownerOnly() {
        uint256 showBal = rewardToken.balanceOf(address(this)).sub(totalStakedShow);
        safeRewardTransfer(_to, showBal);
    }
    ...
    function safeRewardTransfer(address _to, uint256 _amount) internal 
    {
        ...
        lastRewardBalance = rewardToken.balanceOf(address(this)).sub(totalStakedShow);
    }
